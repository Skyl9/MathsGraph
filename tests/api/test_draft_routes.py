...
async def test_create_and_get_draft(async_client: AsyncClient, setup_user_token_admin, setup_test_concept):
    ...
    response = await async_client.post("/drafts/", json=draft_payload, headers=headers)
    ...
    # Get draft
    response = await async_client.get(f"/drafts/{draft_id}", headers=headers)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
async def test_update_draft(async_client: AsyncClient, setup_user_token_admin, setup_test_concept):
    ...
    create_resp = await async_client.post("/drafts/", json=draft_payload, headers=headers)
    draft_id = create_resp.json()["data"]["id"]

    # Update draft
    update_payload = {"draft_data": {"enonce": "Enoncé modifié"}}
    response = await async_client.patch(f"/drafts/{draft_id}", json=update_payload, headers=headers)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
async def test_publish_draft(async_client: AsyncClient, setup_user_token_admin, setup_test_concept):
    ...
    response = await async_client.post(f"/drafts/{draft_id}/publish", headers=headers)
    assert response.status_code == status.HTTP_200_OK

    # Verify draft is deleted
    get_resp = await async_client.get(f"/drafts/{draft_id}", headers=headers)
...
async def test_delete_draft(async_client: AsyncClient, setup_user_token_admin, setup_test_concept):
    ...
    response = await async_client.delete(f"/drafts/{draft_id}", headers=headers)
    assert response.status_code == status.HTTP_200_OK
