...
async def test_get_concept_success(async_client: AsyncClient, setup_full_test_concept):
    ...
    response = await async_client.get(f"/concepts/{concept_id}")
...
async def test_get_concept_not_found(async_client: AsyncClient):
    ...
    """
    response = await async_client.get("/concepts/99999")
...
async def test_get_all_concept_name_success(async_client: AsyncClient, setup_test_concept, setup_full_test_concept):
    ...
    """
    response = await async_client.get("/concepts/getAllConceptName")
    assert response.status_code == 200
    data = response.json()
...
async def test_get_editable_fields_options_success(
    ...
    """
    response = await async_client.get("/concepts/getEditableFieldsOptions")
    assert response.status_code == 200
    data = response.json()
...
async def test_update_concept_simple_field_success(
    ...
    get_response = await async_client.get(f"/concepts/{concept_id}")
    get_data = get_response.json()

    response = await async_client.patch(f"/concepts/{concept_id}", json=update_data, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
    ...
    history_response = await async_client.get(f"/concepts/history/{concept_id}")
...
async def test_update_concept_invalid_id(async_client: AsyncClient, setup_test_user, setup_user_token_admin):
    ...
    response = await async_client.patch("/concepts/99999", json=update_data, headers=headers)
    assert response.status_code == 404  # Si NotFoundException est convertie en InternalServerError
    data = response.json()
    assert data["success"] is False
    assert "Concept non trouvé" in data["error"]
...
async def test_rollback_concept_success(
    ...
    await async_client.patch(f"/concepts/{concept_id}", json=first_update_data, headers=headers)
    ...
    response = await async_client.patch(f"/concepts/rollback/{concept_id}", json=rollback_data, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
    assert data["data"] is None
...
async def test_get_concept_history_success(
    ...
    response = await async_client.get(f"/concepts/history/{concept_id}", headers=headers)
    assert response.status_code == 200
    data = response.json()
...
async def test_create_concept_success(async_client: AsyncClient, setup_user_token_admin, setup_test_type):
    ...
    response = await async_client.post("/concepts", json=payload, headers=headers)
...
async def test_get_recent_history_success(async_client: AsyncClient, setup_full_test_concept):
    ...
    """
    response = await async_client.get("/concepts/recent-history?limit=10")
    assert response.status_code == 200
    data = response.json()
