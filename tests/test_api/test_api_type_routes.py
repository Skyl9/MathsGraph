...
@pytest.mark.asyncio
async def test_update_type(transaction,async_client: httpx.AsyncClient, setup_test_type):
    type_id = setup_test_type["id"]
    payload={
        "value":"new_type",
        "field":"type"
    }
    response = await async_client.patch(f"/type/update/{type_id}", json=payload)
    assert response.status_code == 200
    response_data = response.json()
    assert response_data["success"] is True
...
@pytest.mark.asyncio
async def test_update_type_no_type(transaction,async_client: httpx.AsyncClient):
    type_id = 9999
    payload={
        "value":"new_type",
        "field":"type"
...
@pytest.mark.asyncio
async def test_wrong_field_update_type(transaction,async_client: httpx.AsyncClient, setup_test_type):
    type_id = setup_test_type["id"]
    payload={
        "value":"new_type",
        "field":"wrong_field"
...
@pytest.mark.asyncio
async def test_create_type(transaction,async_client: httpx.AsyncClient, setup_test_user):
    payload = {
        "value":"new_type"
    }
    response = await async_client.post("/type/create", json=payload)
    assert response.status_code == 200
    response_data = response.json()
    assert response_data["success"] is True
    assert "data" in response_data
...
@pytest.mark.asyncio
async def test_create_type_conflict(transaction,async_client: httpx.AsyncClient, setup_test_type):
    payload = {
