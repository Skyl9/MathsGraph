...
async def test_get_type_success(async_client: httpx.AsyncClient, setup_test_type):
    ...
    response = await async_client.get(f"/types/{type_id}")
...
async def test_update_type(
    db_session: AsyncSession, async_client: httpx.AsyncClient, setup_test_type, setup_user_token_admin
):
    headers = create_headers_token(setup_user_token_admin)
    type_id = setup_test_type["id"]
    payload = {"value": "new_type", "field": "type"}
    response = await async_client.patch(f"/types/{type_id}", json=payload, headers=headers)
    assert response.status_code == 200
    response_data = response.json()
    assert response_data["success"] is True
...
@pytest.mark.asyncio
async def test_wrong_field_update_type(
    db_session: AsyncSession, async_client: httpx.AsyncClient, setup_test_type, setup_user_token_admin
):
    headers = create_headers_token(setup_user_token_admin)
    type_id = setup_test_type["id"]
    payload = {"value": "new_type", "field": "wrong_field"}
    response = await async_client.patch(f"/types/{type_id}", json=payload, headers=headers)
...
async def test_get_all_types(db_session: AsyncSession, async_client: httpx.AsyncClient, setup_test_type):
    response = await async_client.get("/types/")
    assert response.status_code == 200
    response_data = response.json()
    assert response_data["success"] is True
    ...
    assert data[0]["nom"] == setup_test_type["type"]


@pytest.mark.asyncio
async def test_create_type(db_session: AsyncSession, async_client: httpx.AsyncClient, setup_user_token_admin):
    headers = create_headers_token(setup_user_token_admin)
    payload = {"value": "new_type"}
    response = await async_client.post("/types", json=payload, headers=headers)
...
@pytest.mark.asyncio
async def test_create_type_conflict(
    db_session: AsyncSession, async_client: httpx.AsyncClient, setup_test_type, setup_user_token_admin
):
    headers = create_headers_token(setup_user_token_admin)
    payload = {"value": setup_test_type["type"]}
    response = await async_client.post("/types", json=payload, headers=headers)
...
async def test_get_type_by_name(db_session: AsyncSession, async_client: httpx.AsyncClient, setup_test_type):
    response = await async_client.get(f"/types/name/{setup_test_type['type']}")
    assert response.status_code == 200
    response_data = response.json()
    assert response_data["success"] is True
    assert "data" in response_data
...
async def test_get_type_by_name_not_found(db_session: AsyncSession, async_client: httpx.AsyncClient, setup_test_type):
    response = await async_client.get("/types/name/wrong_name")
    assert response.status_code == 404
    response_data = response.json()
    assert response_data["success"] is False
    assert "Type introuvable" in response_data["error"]
