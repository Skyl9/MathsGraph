...
async def rollback_concept(concept_id: int, data: RollbackConcept, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user_payload)):
...
async def get_editable_fields_options(db: AsyncSession = Depends(get_db)):
    ...
    logger.debug(f'Route GET /getEditableFieldsOptions a renvoyé correctement la liste des options : {str(editable_field)}')
...
async def get_history(concept_id: int, db: AsyncSession = Depends(get_db)):
    ...
    logger.debug(f'Route /concept/history/{concept_id} a renvoyé correctement la list des versions: {str(history_list)}')
    return {"error": None, "data": history_list, "success": True, "meta": None}


@router.patch("/concept/{concept_id}", response_model=Response)
async def update_concept(concept_id: int, data: UpdateConceptDict, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user_payload)):
    data.username = current_user.get("sub")
