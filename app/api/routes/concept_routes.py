...
async def getConcept(concept_id: int, db: AsyncConnection = Depends(get_db)):
    try:
        ...
        logger.debug(f'Route GET /concept/{concept_id} a renvoyé correctement : {concept}')
        return {"error":None,"data":concept,"success":True,"meta":None}
    except InternalServerError as exc:
        logger.error(f"Route GET /concept/{concept_id} Erreur : {exc}")
...
async def rollbackConcept(concept_id: int, data: RollbackConcept, db: AsyncConnection = Depends(get_db)):
    try:
        ...
        logger.error(f"Route PATCH /concept/rollback/{concept_id} Erreur : {exc}")
        raise InternalServerError(str(exc)) from exc
...
async def getEditableFieldsOptions(db: AsyncConnection = Depends(get_db)):
    try:
        ...
        logger.debug(f'Route GET /getEditableFieldsOptions a renvoyé correctement la liste des options : {editableField}')
        return {"error":None,"data":editableField,"success":True,"meta":None}
    except InternalServerError as exc:
        logger.error(f"Route GET /getEditableFieldsOptions Erreur : {exc}")
        raise InternalServerError(str(exc)) from exc
...
async def getHistory(concept_id: int, db: AsyncConnection = Depends(get_db)):
    try:
        ...
        logger.debug(f'Route /concept/history/{concept_id} a renvoyé correctement la list des versions: {historyList}')
        return {"error":None,"data":historyList,"success":True,"meta":None}
    except InternalServerError as exc:
        logger.error(f"Route GET /concept/history/{concept_id} Erreur : {exc}")
        raise InternalServerError(str(exc)) from exc
...
async def updateConcept(concept_id: int, data: UpdateConceptDict, db: AsyncConnection = Depends(get_db)):
    try:
        ...
        logger.error(f"Route PATCH /update/{concept_id} Erreur : {exc}")
        raise InternalServerError(str(exc)) from exc
...
async def get_all_concept_name_R(db: AsyncConnection = Depends(get_db)):
    try:
        ...
        logger.debug(f'Route /getAllConceptName a renvoyé correctement la liste : {conceptNameList}')
        return {"error":None,"data":conceptNameList,"success":True,"meta":None}
    except InternalServerError as exc:
        logger.error(f"Route GET /getAllConceptName Erreur : {exc}")
        raise InternalServerError(str(exc)) from exc
