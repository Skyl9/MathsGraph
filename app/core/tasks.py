    ...
    while True:
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                stmt_tokens = delete(PasswordResetToken).where(PasswordResetToken.expires_at < now)
                res_tokens = await session.execute(stmt_tokens)

                await session.commit()

                logger.info(
                    f"Nettoyage terminé: {res_sessions.rowcount} sessions expirées, {res_tokens.rowcount} tokens expirés supprimés."
                )
