...
from models import UserCreate, User, Token, ConceptValidation, Comment

from database import get_db_connection

load_dotenv()

ACCESS_TOKEN_EXPIRE_MINUTES = int(load_dotenv('ACCESS_TOKEN_EXPIRE_MINUTES'))
app = FastAPI()
app.add_middleware(
    CORSMiddleware,
...
@app.get("/concepts/{concept_id}/views")
...
async def register_user(user: UserCreate):
    ...
    cursor.execute(
        ...
        VALUES (%s, %s, %s) RETURNING id, username, email, is_active
        """,
...
async def add_comment(concept_id: int, comment: Comment):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("""
                       INSERT INTO comments (concept_id, user_id, content, parent_id)
                       VALUES (%s, %s, %s, %s)
                       RETURNING id, created_at
                       """, (concept_id, comment.user_id, comment.content, comment.parent_id))
...
@app.post("/concepts/{concept_id}/validate")
async def validate_concept(concept_id: int, validation: ConceptValidation):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("""
                       INSERT INTO concept_validations (concept_id, validator_id, status, comments)
                       VALUES (%s, %s, %s, %s)
                       RETURNING id, validated_at
                       """, (concept_id, validation.validator_id, validation.status, validation.comments))
        ...
        conn.commit()
        return {"id": result[0], "validated_at": result[1]}
    except Exception as e:
        conn.rollback()
        raise HTTPException(status_code=400, detail=str(e))
    finally:
...
@app.get("/concepts/{concept_id}/history")
async def get_concept_history(concept_id: int):
    conn = get_db_connection()
    cursor = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    try:
        cursor.execute("""
                       ...
                       FROM concept_history ch
                                JOIN users u ON ch.modified_by = u.id
                       WHERE ch.concept_id = %s
